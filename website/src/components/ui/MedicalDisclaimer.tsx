  ...
  if (compact) {
    return (
      <div className={`p-4 rounded-[12px] bg-[#F2E8DA]/60 border border-[#e6e2dc] text-xs text-[#66615C] flex items-start gap-2.5 ${className}`}>
        ...
        <p>
          <strong className="font-semibold text-[#2D2A26]">Medical Disclaimer:</strong> REVIA provides clinician-reviewed educational content and rehabilitation support. It does not diagnose medical conditions, prescribe medication, replace healthcare professionals, or provide emergency services.
        </p>
      </div>
    );
  }
    ...
    <div className={`p-6 rounded-[16px] bg-[#F2E8DA]/80 border border-[#e6e2dc] text-sm text-[#66615C] flex flex-col md:flex-row items-start gap-4 ${className}`}>
      ...
      <div className="space-y-1">
        ...
        <p className="leading-relaxed">
          REVIA provides clinician-reviewed educational content, routine tracking, and rehabilitation guidance designed to support your recovery journey. <strong className="text-[#2D2A26]">REVIA is not a medical device, does not diagnose conditions, prescribe medications, or replace direct clinical oversight.</strong> Always consult your physician or qualified healthcare provider before starting any rehabilitation or physical activity program. If you experience severe symptoms, chest pain, or a medical emergency, seek immediate medical attention or call emergency services.
        </p>
      </div>
    </div>
  );
